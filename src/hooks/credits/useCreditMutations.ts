...
export function useCreditMutations(userId: string | null, credits: UserCredits | null, refetch: () => Promise<any>) {
  ...
  // Use credits mutation with direct database update
  const useCredits = useMutation({
    mutationFn: async (params: UseCreditsParams): Promise<CreditOperationResult> => {
      ...
      try {
        ...
        let newTotalCreditsUsed = latestCredits.total_credits_used || 0;
        ...
        // Manual immediate refetch to ensure we have updated data
        await refetch();
        
        return { success: true, message: 'Credits updated successfully' };
      } catch (error) {
        console.error('Error in useCredits mutation:', error);
        return { success: false, message: `Error updating credits: ${error}` };
      }
    },
    onSuccess: () => {
      console.log('Credit usage successfully applied');
    },
    onError: (error) => {
  ...
  const purchaseCredits = useMutation({
    mutationFn: async (params: PurchaseCreditsParams): Promise<CreditOperationResult> => {
      ...
      try {
        ...
        console.error('Error in purchaseCredits mutation:', error);
        return { success: false, message: `Error purchasing credits: ${error}` };
      }
    }
  });
