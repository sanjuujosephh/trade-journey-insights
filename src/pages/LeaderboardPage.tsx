...
import { TrendingUp, TrendingDown, ChevronDown, Info, Clock } from "lucide-react";
    ...
    return (
      <div className="space-y-6">
        {entries.map((entry, index) => (
          <Card key={`${entry.username}-${entry.rank}`} className="overflow-hidden">
                  ...
                  <div>
                    ...
                    <div className={`text-sm font-semibold ${isWinners ? 'text-green-600' : 'text-red-600'}`}>
                      {isWinners ? (
                        ...
                        <span className="flex items-center"><TrendingDown className="h-3 w-3 mr-1" /> {formatCurrency(Math.abs(entry.profit_loss))}</span>
                      )}
                    </div>
                  </div>
                </div>
